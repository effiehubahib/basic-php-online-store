...
if(!isset($_GET['day'])){
	$queryRefundedToday = "SELECT tbl_product.product_name as product_name,
		      						...
		      						WHERE DATE(tbl_cart_product.refund_date) = DATE(NOW()) &&
	...
	$queryRefundedToday = "SELECT tbl_product.product_name as product_name,
		      						...
		      						WHERE DATE(tbl_cart_product.refund_date) = DATE('".$_GET['day']."') &&
		      						tbl_cart_product.refunded=1";
}
...
$pdf->Cell(1,0,"Today's Refunded Product: ".date("F d, Y") );
$pdf->Ln(12);
...
if(mysqli_num_rows($resultRefundedToday)>0){
	...
	$pdf->Cell(1,0,"No refunded product today!");
}
