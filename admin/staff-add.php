	...
	if(isset($_POST['submitRegister'])){
		...
		if (isset($username) && preg_match("/\\s/",  $username)) 
		{
	 	...
	 	if($error=='' && $error2=='')
	 	{
			...
			if ($result)
			{	
				echo "<script>alert('Account Registered');window.location='staff.php'</script>";
			}
			else
			{
				$error="Error Saving!";
			...
			<div id="center_column" class="center_column col-xs-10 col-sm-10">
				<form action="" method="post" id="account-creation_form" class="std box">
					...
					<div class="account_creation">
						<h3 class="page-subheading">Create New Staff</h3>
						...
						<div class="required gender">
							...
							<input type="radio" name="gender" value="Male" required="true" <?php if(isset($_POST['gender']) && $_POST['gender']=='Male'){ echo "selected='selected'";}?>>Male
							<input type="radio" name="gender" value="Female" required="true" <?php if(isset($_POST['gender']) && $_POST['gender']=='Female'){ echo "selected='selected'";}?>>Female
						</div>
					...
					<div class="submit clearfix">	
						<button type="submit" name="submitRegister" id="submitRegister" class="btn btn-default button button-medium">
							<span>Add New Staff<i class="icon-chevron-right right"></i></span>
						</button>
					</div>
					<br/>
					<br/>
